...
def setup_preparedStatements():
    try:
        ...
        cursor.execute('prepare getStudentProjectClicks AS select project from sessionprojectclick where sessionid in(select sessionid from session where studentid=$1) limit 50')
